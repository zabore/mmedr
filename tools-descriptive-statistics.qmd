...
Also make sure the breastcancer data are loaded into your current R session, with names standardized using the `clean_names()` function from the {janitor} package, and that you are working inside your mmedr R project.
...
To make the table look even nicer, we could use the `case_match()` function from the {dplyr} package, which works similarly to `case_when()` but only considering a single variable, and is useful for recoding the levels of a categorical variables. We may want to create more descriptive levels than the 1/0 indicators these variables currently contain.
