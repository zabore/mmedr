...
The {dplyr} package is a package specifically designed for data manipulation, and it we will focus on learning functions from this and other related packages in this course.
...
Recall that we can create a subset of our data based on the values in a row, such as limiting to patients who were treated with radiation therapy (i.e. rt = 1):
...
Using the {dplyr} package, we can similarly subset based on rows using the `filter()` functions and based on columns using the `select()` function.
